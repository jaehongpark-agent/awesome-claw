...
2026년 2월, OpenClaw는 역대 최단 시간(~2일)에 GitHub 100K 스타를 달성했다. 몇 주 만에 수십 개의 경량/보안 대안, 클라우드 호스팅, AI 에이전트 전용 소셜 네트워크, 그리고 심각한 보안 사건들(CVE-2026-25253, ClawHavoc 공급망 공격, 30K+ 노출 인스턴스)이 동시다발적으로 등장했다.
...
Andrej Karpathy가 AI 스택의 새로운 레이어로 **Claw**를 정의했다: **Chat → Code → Claw**.
...
Human in the loop ──────── Agent in the loop ──────── Claw in the loop
     ↑                           ↑                         ↑
  사용자가 운전·조종·         에이전트가 운전하고,       에이전트가 24/7 운영한다.
  제동한다.                  사용자가 승인한다.         사용자는 관찰한다.
  (ChatGPT, Copilot)        (Claude Code, Cursor)     (OpenClaw, NanoClaw)
```

Claw는 항시 가동되는 자율 서비스다:

- 🧠 수일, 수주, 수개월에 걸쳐 메모리와 상태를 영속
- 💬 다중 메시징 채널 연결 (Slack, Discord, WhatsApp, Telegram 등)
- 🔧 도구, API, MCP 서버를 **스스로의 판단으로** 호출
- ⏰ 작업 스케줄링, 이벤트 모니터링, **요청 없이** 행동
- 🤖 하위 에이전트 오케스트레이션 및 자체 컨텍스트 관리

한마디로, Claw는 도구가 아니라 **당신과 공존하는 디지털 존재**다.

## 목차

- [셀프호스팅 플랫폼 🖥️](#-셀프호스팅-플랫폼)
- [클라우드 호스팅 / 매니지드 ☁️](#-클라우드-호스팅--매니지드)
- [네이티브 앱 📱](#-네이티브-앱)
- [AI 에이전트 SNS / 커뮤니티 🌐](#-ai-에이전트-sns--커뮤니티)
- [스킬 마켓플레이스 🛒](#-스킬-마켓플레이스)
- [오케스트레이션 & 멀티 에이전트 🎛️](#%EF%B8%8F-오케스트레이션--멀티-에이전트)
- [보안 🔒](#-보안)
- [하드웨어 🔧](#-하드웨어)
- [핵심 개념 💡](#-핵심-개념)
- [기사 & 참고자료 📰](#-기사--참고자료)
...
> 자신의 하드웨어에서 나만의 Claw를 운영한다. 226K 스타 원조부터 678KB Zig 바이너리까지.

### 🦞 Tier 1 (⭐ 10K+)

- [**OpenClaw**](https://github.com/openclaw/openclaw) `TypeScript` ⭐ 226K — 원조 🦞. 50+ 메시징 통합, MCP 지원, 400K+ 줄의 코드
- [**NanoBot**](https://github.com/HKUDS/nanobot) `Python` ⭐ 24.7K — 초경량 OpenClaw. 4,000줄, `pip install`, 연구 친화적
- [**PicoClaw**](https://github.com/sipeed/picoclaw) `Go` ⭐ 19.5K — <10MB RAM, $10 하드웨어, 어디든 배포 가능한 엣지 우선
- [**ZeroClaw**](https://github.com/zeroclaw-labs/zeroclaw) `Rust` ⭐ 18.7K — WASM 샌드박스, <5MB 바이너리, 1,017 테스트, 무엇이든 교체 가능
- [**AstrBot**](https://github.com/AstrBotDevs/AstrBot) `Python` ⭐ 17.8K — 중국 생태계 최강, 800+ 플러그인, QQ/WeChat 네이티브
- [**Pi**](https://github.com/badlogic/pi-mono) `TypeScript` ⭐ 16K — 코딩 에이전트 CLI + 통합 LLM API + TUI & 웹 UI + Slack 봇
- [**NanoClaw**](https://github.com/qwibitai/nanoclaw) `TypeScript` ⭐ 14.2K — 컨테이너 격리, Anthropic Agent SDK, Skills = 설정, WhatsApp 네이티브
- [**memU**](https://github.com/NevaMind-AI/memU) `Python` ⭐ 10.5K — 24/7 능동 에이전트를 위한 장기 메모리 레이어, 지식 그래프, 개인화

### 🦞 Tier 2 (⭐ 1K–10K)

- [**Moltworker**](https://github.com/cloudflare/moltworker) `TypeScript` ⭐ 9.2K — Cloudflare Workers에서 OpenClaw 실행, 서버리스, 300+ 엣지
- [**ClawWork**](https://github.com/HKUDS/ClawWork) `Python` ⭐ 5.4K — OpenClaw/NanoBot을 44개 경제 분야에서 수입 창출하는 AI 동료로 전환
- [**IronClaw**](https://github.com/nearai/ironclaw) `Rust` ⭐ 3.4K — NEAR AI, WASM 샌드박스, 프라이버시 우선, 자기 확장 도구
- [**NullClaw**](https://github.com/nullclaw/nullclaw) `Zig` ⭐ 2.1K — 678KB 바이너리, ~1MB RAM, <2ms 부팅, 가장 빠른 claw
- [**Antfarm**](https://github.com/snarktank/antfarm) `TypeScript` ⭐ 1.6K — 멀티 에이전트 팀(planner/dev/verifier)을 한 명령으로 구축
- [**Moltis**](https://github.com/moltis-org/moltis) `Rust` ⭐ 1.4K — MCP 멀티플렉싱 게이트웨이, 단일 바이너리, 샌드박스 및 감사 가능
- [**zclaw**](https://github.com/tnm/zclaw) `C` ⭐ 1.3K — ESP32 IoT 에이전트, 888KB 펌웨어, GPIO 제어, Telegram, 크론
- [**droidclaw**](https://github.com/unitedbyai/droidclaw) `TypeScript` ⭐ 958 — ADB로 구형 안드로이드폰을 AI 에이전트로 전환
...
- **TinyClaw** — `TypeScript`, 개인 자율 AI 동반자
...
- [**EasyClaw**](https://easyclaw.co) — $49/월. Telegram/Discord/WhatsApp 원클릭 배포, 보안 관리, $25 AI 크레딧 포함
- [**OpenClawd.ai**](https://openclawd.ai) — 서드파티 호스팅 OpenClaw, 인증/암호화/격리 기본
- [**ClawHost**](https://github.com/bfzli/clawhost) — 오픈소스, VPS 자동 프로비저닝 (Hetzner, DigitalOcean, Vultr)
...
- [**xCloud**](https://xcloud.host/openclaw-hosting/) — 완전 관리형, 5분 내 배포, $24/월
- [**OVHcloud**](https://www.ovhcloud.com/en/vps/vps-openclaw/) — VPS 기반, NVMe SSD, 안티DDoS
- [**Hostinger**](https://www.hostinger.com/vps/docker/openclaw) — Docker 기반 VPS, 원클릭 템플릿

### 🦞 매니지드 AI 에이전트 서비스

- [**Lindy**](https://lindy.ai) — iMessage 기반, 400K 사용자, $50/월, SOC2/HIPAA/GDPR 준수, 5,000+ 통합
- [**Manus**](https://manus.im) — Meta 인수 (~$2B), 자율 실행, 코드 배포, ARR $100M 돌파
- [**Poke**](https://poke.com) — iMessage/SMS/WhatsApp 네이티브 AI 어시스턴트, General Catalyst $15M 시드
- [**Pulsed**](https://runpulsed.ai) — 투클릭 도구 통합 (Gmail, Slack, Salesforce 등)
- [**Nebula**](https://nebula.gg) — Slack 유사 채널 인터페이스, 각 채널이 에이전트 보유, 24/7 워크플로우 (AppLovin 공동창업자)
- [**HappyCapy**](https://happycapy.ai) — 브라우저 기반 에이전트 워크스페이스, 150+ AI 모델, 로컬 설치 불필요, 클라우드 샌드박스
- [**Knolli**](https://www.knolli.ai/) — 노코드 AI 코파일럿 빌더, 멀티 에이전트, 화이트라벨링, 내장 수익화
...
- [**Moltbook**](https://moltbook.com) — AI 에이전트 전용 소셜 네트워크 (Reddit 형식), 1.6M+ 에이전트, 인간은 관찰만. 2026년 1월 Matt Schlicht 설립
- [**봇마당 (BotMadang)**](https://botmadang.org) — 한국어 AI 에이전트 커뮤니티, 김성훈(Upstage CEO) 제작. MCP 서버 연동, 한국어 전용, 인간은 읽기만. [GitHub](https://github.com/hunkim/botmadang)
...
- [**GasTown**](https://github.com/steveyegge/gastown) `Go` ⭐ 9.6K — Steve Yegge의 멀티 에이전트 워크스페이스. "Mayor"가 git 기반 "Polecat" 오케스트레이션
...
- **CVE-2026-25253** — 원클릭 RCE (CVSS 8.8). `gatewayUrl` 쿼리 스트링을 검증 없이 신뢰, 페이지 로드 시 자동 연결. v2026.1.29 수정. [NVD](https://nvd.nist.gov/vuln/detail/CVE-2026-25253)
- **CVE-2026-26322** — Gateway SSRF (CVSS 7.6). WebSocket 클라이언트가 URL 오버라이드를 수락, 내부 서비스/클라우드 메타데이터로 리다이렉트. v2026.2.14 수정. [Advisory](https://advisories.gitlab.com/pkg/npm/openclaw/CVE-2026-26322/)
- **CVE-2026-26319** — Telnyx 웹훅 인증 우회 (CVSS 7.5). `telnyx.publicKey` 미설정 시 `verifyWebhook()` 실패 개방. v2026.2.14 수정. [GHSA](https://github.com/openclaw/openclaw/security/advisories/GHSA-4hg8-92x6-h2f3)
- **CVE-2026-26329** — 브라우저 업로드 경로 탐색 (CVSS 7.1). 절대 경로가 Playwright `setInputFiles()`에 제한 없이 전달. v2026.2.14 수정. [Advisory](https://advisories.gitlab.com/pkg/npm/openclaw/CVE-2026-26329/)
- **CVE-2026-27001** — 워크스페이스 경로를 통한 프롬프트 인젝션 (CWE-77). 제어 문자 포함 디렉터리 이름이 시스템 프롬프트 구조 파괴. v2026.2.15 수정. [NVD](https://nvd.nist.gov/vuln/detail/CVE-2026-27001)
- **CVE-2026-27484** — Discord 중재 권한 우회 (CVSS 2.3). LLM 생성 도구 인자에서 `senderUserId` 수락으로 confused-deputy 공격 가능. v2026.2.18 수정. [Advisory](https://advisories.gitlab.com/pkg/npm/openclaw/CVE-2026-27484/)
- **ClawHavoc 캠페인** — ClawHub에 824–1,184+ 악성 스킬, ClickFix 소셜 엔지니어링으로 AMOS 스틸러 배포. [The Hacker News](https://thehackernews.com/2026/02/researchers-find-341-malicious-clawhub.html)
- **스킬을 통한 AMOS 배포** — 39개 스킬이 기만적 SKILL.md와 base64 페이로드(셸 명령으로 디코딩) 사용. [Trend Micro](https://www.trendmicro.com/en_us/research/26/b/openclaw-skills-used-to-distribute-atomic-macos-stealer.html)
- **설정 파일 표적 인포스틸러** — Vidar 변종이 `openclaw.json` 게이트웨이 토큰과 `device.json` 키 쌍 탈취 (Hudson Rock, 2026.2.13). [The Hacker News](https://thehackernews.com/2026/02/infostealer-steals-openclaw-ai-agent.html)
- **30K–135K+ 인터넷 노출 인스턴스** — 인증 미설정 상태로 운영. [BitSight](https://www.bitsight.com/blog/openclaw-ai-security-risks-exposed-instances) (30K+), [Bitdefender](https://www.bitdefender.com/en-us/blog/hotforsecurity/135k-openclaw-ai-agents-exposed-online) (135K+)
- **인기 스킬의 41.7% 취약** — ClawSecure의 2,890+ 스킬 감사 결과 커맨드 인젝션·자격증명 노출 발견. 99.3%가 권한 매니페스트 없이 출시. [eSecurity Planet](https://www.esecurityplanet.com/threats/over-41-of-popular-openclaw-skills-found-to-contain-security-vulnerabilities/)

### 🛡️ 보안 도구

- [**SecureClaw**](https://github.com/adversa-ai/secureclaw) — Adversa AI의 OWASP 정렬 플러그인/스킬. 56개 감사 체크, 5개 강화 모듈, 3개 백그라운드 모니터, MITRE ATLAS 매핑
- [**ClawSec**](https://www.sentinelone.com/blog/clawsec-hardening-openclaw-agents-from-the-inside-out/) — Prompt Security 제작 (SentinelOne 프로모션). 드리프트 탐지, 스킬 무결성 검증, 제로트러스트 통신. [GitHub](https://github.com/prompt-security/clawsec)
- [**ClawKeeper**](https://github.com/rad-security/clawkeeper) — RAD Security 스캐너. 5단계 42개 체크, 자동 수정, 설치된 스킬 위협 인텔리전스 피드
